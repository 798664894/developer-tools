  ...
  <div class="treat-home flex">
    <div class="l-left flex flex-v">
      <div class="sign module-item flex-1">
        <p class="title"><i class="title-icon icon-sign"></i>患者体征</p>
        <sign />
      </div>
      <div class="allergy module-item">
        <p class="title"><i class="title-icon icon-allergy"></i>过敏信息</p>
        <allergy />
      </div>
      <div class="mome module-item flex-1">
        <p class="title"><i class="title-icon icon-mome"></i>患者备注</p>

        <el-popover placement="top" trigger="click" :width="400 * sizeMultiple">
          <span
            v-for="(item, index) in tags"
            :key="index"
            :class="['tag', `color-${index % 8}`]"
            @click="addMome(item.bqmxmc)"
          >
            {{ item.bqmxmc }}
          </span>
          <div class="tag-btn" slot="reference">便捷短语</div>
        </el-popover>
        <div class="edit-btn" @click="showDialog">编辑便捷短语</div>
        <div class="memo-box">
          <textarea
            :class="['mome-content', `fs-${fontSize}`]"
            placeholder="请输入备注"
            v-model="memo"
            @blur="saveMemo"
          >
    ...
    <div class="l-center flex flex-v">
      <div class="case module-item">
        <p class="title" @click="$router.push('/treatInfo/caseHistory')">
          <i class="title-icon icon-case"></i>电子病历
        </p>
        <the-case />
      </div>
      <div class="operation module-item flex-1">
        <p class="title"><i class="title-icon icon-operation"></i>手术</p>
        <operation />
      </div>
    </div>
...
.treat-home {
  ...
  .title-icon {
    ...
    &.icon-allergy {
      background-image: url('../../../assets/images/icon-allergy.png');
    }
    &.icon-mome {
      background-image: url('../../../assets/images/icon-mome.png');
    }
    &.icon-case {
      background-image: url('../../../assets/images/icon-case.png');
    }
    &.icon-operation {
      background-image: url('../../../assets/images/icon-operation.png');
    }
    &.icon-examine {
      background-image: url('../../../assets/images/icon-examine.png');
    }
    &.icon-inspection {
